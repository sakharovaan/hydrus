...
bases = {
    'db/client.caches.db.sql': {},
    'db/client.db.sql': {},
    'db/client.mappings.db.sql': {},
    'db/client.master.db.sql': {}
}
...
def adapt_line(line_bin):
    ...
    if line.startswith('CREATE TABLE'):
        if 'CREATE TABLE urls' in line:
            return 'CREATE TABLE IF NOT EXISTS urls ( url_id INTEGER PRIMARY KEY AUTO_INCREMENT, domain VARCHAR(255), url VARCHAR(2083) CHARACTER SET ascii UNIQUE );'

        line = line.replace('AUTOINCREMENT', 'AUTO_INCREMENT')
        line = line.replace('BLOB_BYTES', 'VARCHAR(255)')
        line = line.replace('INTEGER_BOOLEAN', 'BOOL')
